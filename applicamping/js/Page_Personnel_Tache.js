...
$(document).on("pageshow","#Personnel_Tache_Page",function(){ // When entering pagetwo
		getJsonObject(get_info_taches_perso,'index.php?action=listtaches_personnel&v='+version);

});
$(document).on("pagecreate","#Personnel_Tache_Page",function(){ // When entering pagetwo
	/*
*/
});


function get_info_taches_perso(data){
	data_infos=data;
	$( "#Personnel_Tache_Page").find( "#taches_personnel li" ).remove();
	...
	$("#Personnel_Tache_Page").find( "#taches_personnel" ).append($(htmltaches));
	$("#Personnel_Tache_Page").find("#taches_personnel").listview('refresh');
	$("#Personnel_Tache_Page").trigger('create');

   ...
   function changeInfoExpr(element,etat,mode,fromserver) {
	   ...
	   if (etat!=null && etat['expression']!=null){
		   if (etat['expression']['etat']!=null && typeof etat['expression']['etat'] !='object') {
			   $(element).find("#etat").html(etat['expression']['etat']);
		   }
		   if (etat['expression']['expr1']!=null && typeof etat['expression']['expr1'] !='object') {
			   $(element).find("#expr1").html(etat['expression']['expr1']);
		   }
		   if (etat['expression']['expr1_unit']!=null && typeof etat['expression']['expr1_unit'] !='object') {
			   $(element).find("#expr1_unit").html(etat['expression']['expr1_unit']);
		   }
		   if (etat['expression']['expr2']!=null && typeof etat['expression']['expr2'] !='object') {
			   $(element).find("#expr2").html(etat['expression']['expr2']);
		   }
		   if (etat['expression']['expr3']!=null && typeof etat['expression']['expr3'] !='object') {
   ...
   function changeInfos(element,etat) {
	   ...
	   var lignetexte="";
	   if (etat['expression']) {
		   for (var expr in etat['expression']) {
			   if (expr!="etat"){
				   if (etat['expression'][expr] && typeof(etat['expression'][expr])=="object"){
					   for(var propertyName in etat['expression'][expr]) {
						   if (lignetexte!="") lignetexte+="</br>";
						   lignetexte+=etat['expression'][expr][propertyName];
						   nbinfo++;
						}
				   }   
	   ...
	   if (nbinfo==0) {
		   ...
		   $(element).find("#expr1").html(lignetexte);
		   $(element).parent().show();
	   }
   };
  ...
  function waitAndSendAction(uuid,urlaction,time){
   ...
   function changeEtatPlayer(element,val,fromserver,urlimage) {
	   
	   //$(element).find("#uuid").html($(element).attr('id'));
	   ele=$(element).attr('id')+"";
	   uuid=element.substring(2, 99);/*eneleve la lettre de prefixe*/
	   var objprinc=$('#'+ele).closest( " [obj='princ']" )
	   var coloricon_suffixe="";
	   var pp=peripheriques[uuid];
       if (urlimage && urlimage!=null && typeof urlimage !='object' && uuid){
    	   
    	   if (pp.categorie && pp.categorie.couleur_text=='noir') {
    		   objprinc.css('background-image','linear-gradient(to right, rgba(255,255,255,0.3), rgba(255,255,255,0.3)),url(/index.php?type=piping&action=getplayerimage&uuid='+uuid+"&md5img="+ calcMD5(urlimage)+')');
    	   } else {
    		   objprinc.css('background-image','linear-gradient(to right, rgba(0,0,0,0.3), rgba(0,0,0,0.3)),url(/index.php?type=piping&action=getplayerimage&uuid='+uuid+"&md5img="+ calcMD5(urlimage)+')');
    	   }
    	   objprinc.css('background-color',"");
    	   objprinc.css('background-blend-mode','multiply');
    	   objprinc.css('background-repeate','no-repeat');
    	   objprinc.css('background-size','cover');
    	   objprinc.css('background-position','center');
       } else {
    	   if (pp.categorie && pp.categorie.couleur_fond) {
    		   objprinc.css('background',pp.categorie.couleur_fond);
    	   }
    	   objprinc.css('background-blend-mode','');
    	   objprinc.css('background-repeate','');
    	   objprinc.css('background-size','');
    	   objprinc.css('background-position','');
    	   objprinc.css('background-image','');
       }
	   if (val=='UP' ) 
       {

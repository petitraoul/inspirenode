var lasttagclicked_uuid="";

$(document).on("pagecreate","#periphpage",function(){ // When entering pagetwo
	  //alert('test create call');
	  //displayWaitPeriph();
		//console.log('bind event page hideshow cam');
		$(document).unbind("pagehide",this,pagehidecam);
		$(document).unbind("pageshow",this,pageshowcam);
		$(document).bind("pagehide",this,pagehidecam);
		$(document).bind("pageshow",this,pageshowcam);
	});
...
$(document).on("pageshow","#periphpage",function(){ // When entering pagetwo
	...
	//$(page).trigger('create');
	  ...
	  if (lasttagclicked_uuid!=tagclicked_uuid){
	    ...
	    $("#periphpage").trigger('create');	
	    lasttagclicked_uuid=tagclicked_uuid;
	  }
		...
		if (index!='tags') {
				...
				htmlperiph='<li  data-icon="false">'+
					'<a href="#camerapage" id="'+prefixedid+periph.uuid+'" obj="princ" style="padding:0">'+
				    	...
				    	//'<div><p>'+
				        	...
				        	'<div><img id="cam" src_old="'+cameraURL+'" src="'+cameraURL+"&keep=1&time="+ new Date().getTime()+'" style="width:100%;max-height:100%"/></div>'+
				        	/*'+cameraURL+"&time="+ new Date().getTime()+'*/
				        //'</p><div>'+
			...
			if (listviewname1==listviewname2 && listviewname2==listviewname3) {
				...
				if (periph.ecriture_type=='TEMPERATURE' || periph.ecriture_type=='TEMPERATURECONSIGNE' || periph.ecriture_type=='ALARME'
					|| periph.ecriture_type=='INFOS' || periph.ecriture_type=='PLAYER'/*countplace%2==0*/) {
					$(listviewname1).parent().show();
					$(listviewname1).append(jqueryhtmlperiph);	
					countplace++;
...
function pagehidecam(){
	//console.log('pagehide peripgpage');
	//var idcam=prefixedid+periph.uuid;
	var camurl=cameraURL;
	$(this).find('#cam').find('#cam').attr('src',"");
	//alert($(page).find('#'+idcam).find('#cam').attr('src'));
};
function pageshowcam(){
	//var idcam=prefixedid+periph.uuid;
	var camurl=cameraURL;
	//console.log('pageshow peripgpage');
	if ($(this).find('#cam').attr('src')=="" || $(this).find('#cam').attr('src')=="") {
		$(this).find('#cam').attr('src',$("#periphpage").find('#cam').attr('src_old')+"&keep=1&time="+ new Date().getTime());
		//alert(page);
	}
};